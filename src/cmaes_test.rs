...
mod tests {
    ...
    fn test_cmaes_global_approach_simple_sphere() {
        ...
        let population = cmaes_global_approach(&objective, &bounds, pop_size, max_gen, 42_u64);
        ...
        assert!(
            best.0 < 1e-3,
            "Best objective should be ~0 for Sphere function"
        );
    }
    ...
    fn test_cmaes_global_approach_rosenbrock() {
        ...
        let population = cmaes_global_approach(
            &objective, &bounds, pop_size, max_gen, 123_u64, // Different seed
        );
    ...
    fn test_cmaes_optimize_rosenbrock() {
        ...
        // Verify coordinates
        assert!(
            result.best_solution.1[0] > 0.0,
            "x should be positive, moving toward 1.0"
        );
        assert!(
            result.best_solution.1[1] > 0.0,
            "y should be positive, moving toward 1.0"
        );
    }
    ...
    fn test_cmaes_global_approach_constrained() {
        ...
        let population = cmaes_global_approach(&objective, &bounds, pop_size, max_gen, 789_u64);
        ...
        let best = population
            ...
            .unwrap();
        assert!(
            (best.1[0] - 1.0).abs() < 1e-1,
            "Best x should be close to boundary at 1.0, got {}",
            best.1[0]
        );
    }
